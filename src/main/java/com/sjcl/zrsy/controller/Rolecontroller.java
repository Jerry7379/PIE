...
public class Rolecontroller {
    ...
    @PostMapping("/login")
    @ResponseBody
    public String login(@RequestBody LoginUser loginUser, HttpSession session)  {
        List<String> infos= roleService.login(loginUser);
        String info[]=infos.get(0).split(";");
        if(!info[0].equals(loginUser.getPassword()))
       {
           return "此账户不存在，请重输入";
           ...
           if (info[0].equals(loginUser.getPassword())) {
               session.setAttribute("userInfo", loginUser.getName());
